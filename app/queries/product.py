...
async def add_product(name: str, description: str, price: int, tag_id: dict, urls: dict) -> int:
    ...
    return await DB.con.fetchval(sql, name, description, price, json.dumps(urls), json.dumps(tag_id))
...
async def update_product(prod: ProductUp) -> None:
    sql = '''  UPDATE product
                ...
                WHERE product.id = $5; '''
    if not await DB.con.execute(sql, prod.name, prod.discription, prod.price, json.dumps(prod.urls), prod.product_id):
...
async def get_info_product(product_id: list, login: str) -> list[Record]:
    ...
    return await DB.con.fetch(sql, product_id, customer_id)
